...
def main():
    ...
    # Training
    parser.add_argument('--batch', '--batch-size', "-b", default=32, type=int, metavar='N',
    ...
    parser.add_argument('--selection_lr', '-slr', type=float, default=0.001, help='learning rate for selection')  #previous was 0.1
    ...
    parser.add_argument('--precalcfeatures_path', type= str, default= '')
    ...
    for exp in range(start_exp, args.num_exp):
        ...
        if args.save_path != "":
            if args.uncertainty == "Entropy":
                checkpoint_name = "{dst}_{net}_{mtd}_exp{exp}_{fr}_".format(dst=args.dataset,
                                                                                            net=args.model,
                                                                                            mtd=args.selection,
                                                                                            exp=exp,
                                                                                            fr=args.fraction)
            else:
                checkpoint_name = "{dst}_{net}_{mtd}_{sel}_exp{exp}_{fr}_".format(dst=args.dataset,
                                                                                            net=args.model,
                                                                                            mtd=args.selection,
                                                                                            sel=args.uncertainty,
                                                                                            exp=exp,
                                                                                    fr=args.fraction)
