...
policies= ['difficult', 'easy','random','median']
for method in methods:
    for fraction in fractions:
        for policy in policies:
            sp= '/n/fs/dk-diffusion/repos/DeepCore/checkpoints_SGD/'
            # score_path= f'/n/fs/dk-diffusion/repos/DeepCore/all_results_SGD/waterbirds_pretrained/waterbirds_ResNet50_{method}_exp0_0.1_unknown.ckpt'
            os.system(f"bash run_waterbirds_test.sh {fraction} {method} {sp} {policy}")
            # time.sleep(1)       
            # break
        # break
    # break
