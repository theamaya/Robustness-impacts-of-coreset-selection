...
for i in range(1): #add more for robustness
    for j in range(7): 
        if j== 1:
            for k in range(3):
                os.system(f"sbatch run_waterbirds.sh {coreset_methods[j]} {uncertainty_methods[k]}")
                time.sleep(1)
        else:
            os.system(f"sbatch run_waterbirds.sh {coreset_methods[j]} {uncertainty_methods[0]}")
        time.sleep(1)