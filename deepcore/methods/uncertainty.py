...
class Uncertainty(EarlyTrain):
    ...
    def after_loss(self, outputs, loss, targets, batch_inds, epoch):
        if self.selection_method == "Entropy":
            preds = torch.nn.functional.softmax(outputs.detach(), dim=1).cpu().numpy()
            self.score_tracker[epoch, batch_inds]= -1*(np.log(preds + 1e-6) * preds).sum(axis=1)
    ...
    def finish_run(self):
        ...
        if self.balance:
            ...
            # scores = self.score_tracker[-1,:]
            selection_result = np.argsort(scores)[::-1][:self.coreset_size]
        return {"indices": selection_result, "scores": scores, "score_tracker": self.score_tracker}
        ...
        with torch.no_grad():
            ...
            for i, (input, _, _, _) in enumerate(train_loader):
                ...
                if self.selection_method == "LeastConfidence":
                    ...
                    max_preds = preds[torch.ones(preds.shape[0], dtype=bool), preds_argmax].cpu().numpy()
                    ...
                    scores = np.append(scores, -1*(np.log(preds + 1e-6) * preds).sum(axis=1))
                    ...
                    scores = np.append(scores, 1-(max_preds - preds[
