...
class EarlyTrain(CoresetMethod):
    ...
    def __init__(self, dst_train, args, fraction=0.5, random_seed=None, epochs=200, specific_model=None,
                 torchvision_pretrain: bool = False, dst_pretrain_dict: dict = {}, fraction_pretrain=1., dst_test=None, linear_probe: bool = False,
                 **kwargs):
    ...
    def train(self, epoch, list_of_train_idx, **kwargs): ##training with mixed precision
        self.before_train()
        self.model.train()
    ...
    def train_nlp(self, epoch, list_of_train_idx, **kwargs):
    ...
    def run(self):
        ...
        for epoch in range(self.epochs):
            ...
            self.before_epoch()
            if self.dst_train.data_type == "text":
                self.train_nlp(epoch, list_of_train_idx)
            else:
        ...
        if self.args.save_model:
            pass
            # torch.save(self.model, '/n/fs/dk-diffusion/repos/DeepCore/checkpoints_pretrained/finetune_after_linearprobe/nico95underrep_resnet18/'+str(self.random_seed)+'.pt')
            #/n/fs/dk-diffusion/repos/DeepCore/checkpoints_pretrained/finetune/nico95underrep_resnet18/
    ...
    def test(self, epoch):
        ...
        for batch_idx, (input, target, _, _) in enumerate(test_loader):
