...
def ResNet(arch: str, channel: int, num_classes: int, im_size, record_embedding: bool = False, no_grad: bool = False,
           pretrained: bool = False, linear_probe: bool = False, url: str = None):
    arch = arch.lower()
    if pretrained:
        ...
        # if arch == "resnet50":
        #     checkpoint= torch.hub.load('yukimasano/PASS:main', 'moco_resnet50')
        ...
        ###### remove this later
        # checkpoint= torch.load('/n/fs/dk-diffusion/repos/DeepCore/checkpoints_pretrained/linear_probe/nico95underrep_resnet18/1.pt')
        # net.load_state_dict(checkpoint.state_dict())
        # print('linear probe weights loaded')
        ######
    ...
    if linear_probe:
        ...
        #disable gradients for all except last layer (net.fc or net.linear)
...
def ResNet18(channel: int, num_classes: int, im_size, record_embedding: bool = False, no_grad: bool = False,
    ...
    return ResNet("resnet18", channel, num_classes, im_size, record_embedding, no_grad, pretrained, linear_probe)
...
def ResNet50(channel: int, num_classes: int, im_size, record_embedding: bool = False, no_grad: bool = False,
             pretrained: bool = False, linear_probe: bool = False):
    return ResNet("resnet50", channel, num_classes, im_size, record_embedding, no_grad, pretrained, linear_probe)
